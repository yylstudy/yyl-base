 ...
 * @version 1.0.0
 * @Description TODO
 * @createTime 2024/5/6 10:55
 */

public class CustomDefaultRocketMQListenerContainer implements InitializingBean,
        RocketMQListenerContainer, SmartLifecycle, ApplicationContextAware {
    private final static Logger log = LoggerFactory.getLogger(CustomDefaultRocketMQListenerContainer.class);
    ...
    public CustomDefaultRocketMQListenerContainer setMessageConverter(MessageConverter messageConverter) {
        this.messageConverter = messageConverter;
        return this;
    }

    public MqListener getRocketMQListener() {
        return mqListener;
    }

    public void setRocketMQListener(MqListener mqListener) {
    ...
    public MqMessageListener getRocketMQMessageListener() {
        return mqMessageListener;
    }

    public void setRocketMQMessageListener(MqMessageListener anno) {
        this.mqMessageListener = anno;
        RocketmqConsumeMode rocketmqConsumeMode = anno.consumeMode();
        if(rocketmqConsumeMode==RocketmqConsumeMode.CONCURRENTLY){
        ...
        this.selectorExpression = anno.tags();
    ...
    private MethodParameter getMethodParameter() {
        Class<?> targetClass;
        if (mqListener != null) {
            targetClass = AopProxyUtils.ultimateTargetClass(mqListener);
        } else {
            ...
            if (Objects.nonNull(interfaces)) {
                for (Type type : interfaces) {
                    if (type instanceof ParameterizedType &&
                            (Objects.equals(((ParameterizedType) type).getRawType(), MqListener.class) || Objects.equals(((ParameterizedType) type).getRawType(), RocketMQReplyListener.class))) {
    ...
    private void initRocketMQPushConsumer() throws MQClientException {
        if (mqListener == null && rocketMQReplyListener == null) {
        ...
        RPCHook rpcHook = RocketMQUtil.getRPCHookByAkSk(applicationContext.getEnvironment(),
                MqConstant.ACCESS_KEY_PLACEHOLDER,MqConstant.SECRET_KEY_PLACEHOLDER);
        boolean enableMsgTrace = false;
        if (Objects.nonNull(rpcHook)) {
            consumer = new DefaultMQPushConsumer(consumerGroup, rpcHook, new AllocateMessageQueueAveragely(),
                    enableMsgTrace, this.applicationContext.getEnvironment().
                    resolveRequiredPlaceholders(MqConstant.TRACE_TOPIC_PLACEHOLDER));
        ...
        String customizedNameServer = this.applicationContext.getEnvironment().resolveRequiredPlaceholders(MqConstant.NAME_SERVER_PLACEHOLDER);
        ...
        switch (consumeMode) {
            case ORDERLY:
                consumer.setMessageListener(new CustomDefaultRocketMQListenerContainer.DefaultMessageListenerOrderly());
                break;
            case CONCURRENTLY:
                consumer.setMessageListener(new CustomDefaultRocketMQListenerContainer.DefaultMessageListenerConcurrently());
                break;
            default:
                throw new IllegalArgumentException("Property 'consumeMode' was wrong.");
        }

        if (mqListener instanceof RocketMQPushConsumerLifecycleListener) {
            ((RocketMQPushConsumerLifecycleListener) mqListener).prepareStart(consumer);
