 ...
 * @version 1.0.0
 * @Description 自定义RabbitListenerAnnotationBeanPostProcessor，参考RabbitListenerAnnotationBeanPostProcessor
 * @createTime 2024/4/30 15:38
 */
@Configuration
@ConditionalOnClass({ RabbitTemplate.class, Channel.class })
public class CustomRabbitListenerAnnotationBeanPostProcessor implements BeanPostProcessor, Ordered, BeanFactoryAware, BeanClassLoaderAware, EnvironmentAware,
    ...
    /**
     * RabbitListenerAnnotationBeanPostProcessor 从0开始，所以这边从10000开始，避免冲突
     */
    ...
    public CustomRabbitListenerAnnotationBeanPostProcessor() {
    ...
    public Object postProcessAfterInitialization(final Object bean, final String beanName) throws BeansException {
        ...
        final TypeMetadata metadata = this.typeCache.computeIfAbsent(targetClass, this::customBuildMetadata);
        for (ListenerMethod lm : metadata.listenerMethods) {
            for (MqMessageListener rabbitListener : lm.annotations) {
    ...
    private TypeMetadata buildMetadata(Class<?> targetClass) {
        List<MqMessageListener> classLevelListeners = findListenerAnnotations(targetClass);
        ...
        ReflectionUtils.doWithMethods(targetClass, method -> {
            ...
            List<MqMessageListener> listenerAnnotations = findListenerAnnotations(method);
            if (listenerAnnotations.size() > 0) {
                methods.add(new ListenerMethod(method,
                        listenerAnnotations.toArray(new MqMessageListener[listenerAnnotations.size()])));
            }
        ...
        return new TypeMetadata(
                ...
                classLevelListeners.toArray(new MqMessageListener[classLevelListeners.size()]));
    }

    /**
     * 自定义获取元数据方法，只获取类上注解MqListener中的onMessage方法
     * @param targetClass
     * @return
     */
    private TypeMetadata customBuildMetadata(Class<?> targetClass) {
        List<MqMessageListener> classLevelListeners = findListenerAnnotations(targetClass);
        final boolean hasClassLevelListeners = classLevelListeners.size() > 0;
        if(!hasClassLevelListeners){
            return TypeMetadata.EMPTY;
        ...
        ReflectionUtils.doWithMethods(targetClass, method -> {
            ...
            if(MqConstant.DEFAULT_LISTENER_METHOD.equals(method.getName())){
                methods.add(new ListenerMethod(method,
                        classLevelListeners.toArray(new MqMessageListener[classLevelListeners.size()])));
            }
        }, ReflectionUtils.USER_DECLARED_METHODS);
    ...
    private List<MqMessageListener> findListenerAnnotations(AnnotatedElement element) {
        return MergedAnnotations.from(element, MergedAnnotations.SearchStrategy.TYPE_HIERARCHY)
                .stream(MqMessageListener.class)
                .filter(tma -> {
    ...
    private void processMultiMethodListeners(MqMessageListener[] classLevelListeners, Method[] multiMethods,
        ...
        for (MqMessageListener classLevelListener : classLevelListeners) {
    ...
    protected Collection<Declarable> processAmqpListener(MqMessageListener rabbitListener, Method method, Object bean,
                                                         String beanName) {
    ...
    protected Collection<Declarable> processListener(MethodRabbitListenerEndpoint endpoint,
                                                     MqMessageListener rabbitListener, Object bean, Object target, String beanName) {
        ...
        endpoint.setConcurrency(resolveExpressionAsStringOrInteger("", "concurrency"));
        endpoint.setBeanFactory(this.beanFactory);
        endpoint.setReturnExceptions(resolveExpressionAsBoolean(""));
        ...
        String priority = resolveExpressionAsString("", "priority");
    ...
    private void resolveErrorHandler(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener) {
        Object errorHandler = resolveExpression("");
    ...
    private void resolveAckMode(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener) {
    ...
    private void resolveAdmin(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener, Object adminTarget) {
        Object resolved = resolveExpression("");
        if (resolved instanceof AmqpAdmin) {
            endpoint.setAdmin((AmqpAdmin) resolved);
        }
        else {
            String rabbitAdmin = resolveExpressionAsString("", "admin");
    ...
    @Nullable
    private RabbitListenerContainerFactory<?> resolveContainerFactory(MqMessageListener rabbitListener,
                                                                      Object factoryTarget, String beanName) {
        return null;
    }

    private void resolveExecutor(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener,
        ...
        if (resolved instanceof TaskExecutor) {
            ...
            String execBeanName = resolveExpressionAsString("", "executor");
    ...
    private void resolvePostProcessor(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener,
        ...
        if (resolved instanceof ReplyPostProcessor) {
            ...
            String ppBeanName = resolveExpressionAsString("", "replyPostProcessor");
    ...
    private void resolveMessageConverter(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener,
                                         Object target, String beanName) {
        ...
        if (resolved instanceof MessageConverter) {
            ...
            String mcBeanName = resolveExpressionAsString("", "messageConverter");
    ...
    private void resolveReplyContentType(MethodRabbitListenerEndpoint endpoint, MqMessageListener rabbitListener) {
        String contentType = resolveExpressionAsString("", "replyContentType");
        if (StringUtils.hasText(contentType)) {
            endpoint.setReplyContentType(contentType);
            endpoint.setConverterWinsContentType(resolveExpressionAsBoolean("true"));
        }
    }
    ...
    private String getEndpointId(MqMessageListener rabbitListener) {
        return "org.springframework.amqp.rabbit.RabbitListenerEndpointContainer#" + this.counter.getAndIncrement();
    }

    private List<Object> resolveQueues(MqMessageListener rabbitListener, Collection<Declarable> declarables) {
        String[] queues = new String[]{rabbitListener.target()};
        QueueBinding[] bindings = new QueueBinding[0];
        org.springframework.amqp.rabbit.annotation.Queue[] queuesToDeclare = new org.springframework.amqp.rabbit.annotation.Queue[0];
    ...
    private String[] registerBeansForDeclaration(MqMessageListener rabbitListener, Collection<Declarable> declarables) {
        List<String> queues = new ArrayList<String>();
        if (this.beanFactory instanceof ConfigurableBeanFactory) {
            for (QueueBinding binding : new QueueBinding[0]) {
    ...
    private void registerBindings(QueueBinding binding, String queueName, String exchangeName, String exchangeType,
        ...
        for (String routingKey : routingKeys) {
            final Binding actualBinding = new Binding(queueName, Binding.DestinationType.QUEUE, exchangeName, routingKey,
                    bindingArguments);
    ...
    private class RabbitHandlerMethodFactoryAdapter implements MessageHandlerMethodFactory {
        ...
        private MessageHandlerMethodFactory createDefaultMessageHandlerMethodFactory() {
            ...
            Validator validator = CustomRabbitListenerAnnotationBeanPostProcessor.this.registrar.getValidator();
            if (validator != null) {
                defaultFactory.setValidator(validator);
            }
            defaultFactory.setBeanFactory(CustomRabbitListenerAnnotationBeanPostProcessor.this.beanFactory);
            this.defaultFormattingConversionService.addConverter(
                    new CustomRabbitListenerAnnotationBeanPostProcessor.BytesToStringConverter(CustomRabbitListenerAnnotationBeanPostProcessor.this.charset));
            ...
            List<HandlerMethodArgumentResolver> customArgumentsResolver = new ArrayList<>(
                    CustomRabbitListenerAnnotationBeanPostProcessor.this.registrar.getCustomMethodArgumentResolvers());
    ...
    /**
     * The metadata holder of the class with {@link MqMessageListener}
     * and {@link RabbitHandler} annotations.
     */
    private static class TypeMetadata {

        /**
         * Methods annotated with {@link MqMessageListener}.
         */
        ...
        /**
         * Class level {@link MqMessageListener} annotations.
         */
        final MqMessageListener[] classAnnotations; // NOSONAR
        ...
        private TypeMetadata() {
            ...
            this.classAnnotations = new MqMessageListener[0];
        }

        TypeMetadata(ListenerMethod[] methods, Method[] multiMethods, MqMessageListener[] classLevelListeners) { // NOSONAR
    ...
    /**
     * A method annotated with {@link MqMessageListener}, together with the annotations.
     *
    ...
    private static class ListenerMethod {
        ...
        final MqMessageListener[] annotations; // NOSONAR

        ListenerMethod(Method method, MqMessageListener[] annotations) { // NOSONAR
            this.method = method;
