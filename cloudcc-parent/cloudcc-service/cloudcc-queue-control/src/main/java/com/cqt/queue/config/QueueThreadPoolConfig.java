...
public class QueueThreadPoolConfig {
    ...
    public static final String IDLE_POOL_NAME = "idle-pool";

    public static final String IDLE_SKILL_POOL_NAME = "idles-skill-pool";
    ...
    @Bean(IDLE_POOL_NAME)
    public Executor idlePollExecutor() {
        log.info("初始化 {} 线程池, 当前核数: {}", IDLE_POOL_NAME, CPU_CORE);
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(CPU_CORE);
        executor.setMaxPoolSize(CPU_CORE * 3);
        executor.setQueueCapacity(200);
        executor.setKeepAliveSeconds(60);
        executor.setThreadNamePrefix(IDLE_POOL_NAME);
        executor.setTaskDecorator(new MdcTaskDecorator());
        executor.setWaitForTasksToCompleteOnShutdown(true);
        executor.setPrestartAllCoreThreads(true);
        // 拒绝策略, 由主线程执行
    ...
    @Bean(IDLE_SKILL_POOL_NAME)
    public Executor idleSkillPollExecutor() {
        log.info("初始化 {} 线程池, 当前核数: {}", IDLE_SKILL_POOL_NAME, CPU_CORE);
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(CPU_CORE);
        executor.setMaxPoolSize(CPU_CORE * 3);
        executor.setQueueCapacity(200);
        executor.setKeepAliveSeconds(60);
        executor.setThreadNamePrefix(IDLE_SKILL_POOL_NAME);
        executor.setTaskDecorator(new MdcTaskDecorator());
        executor.setWaitForTasksToCompleteOnShutdown(true);
        executor.setPrestartAllCoreThreads(true);
        // 拒绝策略, 由主线程执行
