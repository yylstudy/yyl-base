...
public class MakeBusyActionStrategyImpl extends AbstractAgentStatusChecker implements AgentStatusTransferStrategy {
    ...
    public ClientChangeStatusVO deal(ClientChangeStatusDTO clientChangeStatusDTO) throws Exception {
        ...
        // 判断当前状态是否是BUSY
        if (AgentStatusEnum.BUSY.name().equals(agentStatusDTO.getTargetStatus())) {
            return ClientChangeStatusVO.response(clientChangeStatusDTO, "1", "当前坐席已经是忙碌状态, 示忙失败!");
        }
        ...
        agentStatusDTO.setTransferAction(AgentStatusTransferActionEnum.MAKE_BUSY.name());
        ...
        log.info("[示忙] 企业坐席空闲队列-删除, 企业: {}, 坐席: {}", companyCode, agentId);
        ...
        return ClientChangeStatusVO.response(clientChangeStatusDTO, agentStatusDTO, "0", "示忙成功!");
    }

    @Override
    public AgentStatusTransferActionEnum getTransferActionEnum() {
        return AgentStatusTransferActionEnum.MAKE_BUSY;
    }
}
