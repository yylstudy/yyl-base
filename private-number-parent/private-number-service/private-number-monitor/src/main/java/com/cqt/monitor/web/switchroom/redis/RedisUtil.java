...
import static com.cqt.monitor.common.util.SignUtil.MINUS;
...
public class RedisUtil {
    ...
    public static RedisClient buildByConfigs(List<MonitorConfigInfo> redisConfigList){
        Set<HostAndPort> hostAndPortSet = new HashSet<>();
        for (MonitorConfigInfo config:redisConfigList) {
            hostAndPortSet.add(new HostAndPort(config.getHost(), config.getPort()));
        }
        RedisUri uri = new RedisUri();
        uri.setDatabase(0);
        uri.setRequirePass(redisConfigList.get(0).getPassword());
        uri.setHostAndPortSet(hostAndPortSet);
        return RedisClientFactory.buildRedisClient(uri);
    }
}
