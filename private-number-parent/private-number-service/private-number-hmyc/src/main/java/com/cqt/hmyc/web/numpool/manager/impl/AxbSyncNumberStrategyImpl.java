    ...
    public Result sync(NumberChangeSyncDTO numberChangeSyncDTO) {
        ...
        if (OperateTypeEnum.DELETE.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolAxbCache.removeAll(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {
                slaveNumberMap.forEach((areaCode, list) -> NumberPoolAxbCache.removeAll(vccId, areaCode, list));
            }
        }
        ...
        if (OperateTypeEnum.INSERT.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                masterNumberMap.forEach((areaCode, list) -> {
                    NumberPoolAxbCache.addPool(AxbPoolTypeEnum.ALL, vccId, areaCode, new HashSet<>(list));
                    NumberPoolAxbCache.addPool(AxbPoolTypeEnum.MASTER, vccId, areaCode, new HashSet<>(list));
                });
            }
            ...
            if (CollUtil.isNotEmpty(slaveNumberMap)) {
                slaveNumberMap.forEach((areaCode, list) -> {
                    NumberPoolAxbCache.addPool(AxbPoolTypeEnum.ALL, vccId, areaCode, new HashSet<>(list));
                    NumberPoolAxbCache.addPool(AxbPoolTypeEnum.SLAVE, vccId, areaCode, new HashSet<>(list));
                });
            }
        }
