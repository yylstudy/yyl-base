...
import com.cqt.hmyc.web.cache.NumberPoolAxgCache;
...
@Slf4j
@Service
public class AxgSyncNumberStrategyImpl implements SyncNumberStrategy {

    public static final String BUSINESS_TYPE = BusinessTypeEnum.AXG.name();
    ...
    public Result sync(NumberChangeSyncDTO numberChangeSyncDTO) {
        ...
        if (OperateTypeEnum.DELETE.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolAxgCache.removeAll(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {

                slaveNumberMap.forEach((areaCode, list) -> NumberPoolAxgCache.removeAll(vccId, areaCode, list));
            }
        }
        ...
        if (OperateTypeEnum.INSERT.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolAxgCache.addPool(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {

                slaveNumberMap.forEach((areaCode, list) -> NumberPoolAxgCache.addPool(vccId, areaCode, list));
            }
        }
