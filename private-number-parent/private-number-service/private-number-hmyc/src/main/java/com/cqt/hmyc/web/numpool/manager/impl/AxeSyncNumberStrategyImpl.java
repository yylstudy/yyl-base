    ...
    public Result sync(NumberChangeSyncDTO numberChangeSyncDTO) {
        ...
        if (OperateTypeEnum.DELETE.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolAxeCache.removeAll(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {
                slaveNumberMap.forEach((areaCode, list) -> NumberPoolAxeCache.removeAll(vccId, areaCode, list));
            }
        }
        ...
        if (OperateTypeEnum.INSERT.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolAxeCache.addPool(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {
                slaveNumberMap.forEach((areaCode, list) -> NumberPoolAxeCache.addPool(vccId, areaCode, list));
            }
        }
