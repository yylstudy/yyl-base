...
import com.cqt.hmyc.web.cache.NumberPoolOutCache;
...
@Slf4j
@Service
public class OutSyncNumberStrategyImpl implements SyncNumberStrategy {
    ...
    public Result sync(NumberChangeSyncDTO numberChangeSyncDTO) {
        ...
        if (OperateTypeEnum.DELETE.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolOutCache.removeAll(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {

                slaveNumberMap.forEach((areaCode, list) -> NumberPoolOutCache.removeAll(vccId, areaCode, list));
            }
        }
        ...
        if (OperateTypeEnum.INSERT.name().equals(numberChangeSyncDTO.getOperationType())) {
            if (CollUtil.isNotEmpty(masterNumberMap)) {
                ...
                masterNumberMap.forEach((areaCode, list) -> NumberPoolOutCache.addPool(vccId, areaCode, list));
            }
            if (CollUtil.isNotEmpty(slaveNumberMap)) {

                slaveNumberMap.forEach((areaCode, list) -> NumberPoolOutCache.addPool(vccId, areaCode, list));
            }
        }
