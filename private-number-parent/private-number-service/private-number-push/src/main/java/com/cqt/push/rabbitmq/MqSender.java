...
public class MqSender {
    ...
    public void sendCheckLazy(Object message,Integer minute) {
        rabbitTemplate.setMandatory(false);
        rabbitTemplate.setConfirmCallback(confirmCallback);
        rabbitTemplate.setReturnCallback(returnCallback);
        ...
        //发送消息时指定 header 延迟时间
        rabbitTemplate.convertAndSend(RabbitMqConfig.CHECK_BILL_PUSH_EXCHANGE, RabbitMqConfig.ROUTING_KEY, message,
                new MessagePostProcessor() {
                    @Override
                    public Message postProcessMessage(Message message) throws AmqpException {
                        //设置消息持久化
                        message.getMessageProperties().setDeliveryMode(MessageDeliveryMode.PERSISTENT);
                        //message.getMessageProperties().setHeader("x-delay", "6000");
                        message.getMessageProperties().setDelay(minute * 60 * 1000);
                        return message;
                    }
                }, correlationData);
    }
