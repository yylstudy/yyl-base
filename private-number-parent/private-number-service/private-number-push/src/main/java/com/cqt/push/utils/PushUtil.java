...
public class PushUtil {
    ...
    public static PrivateStatusInfo buildStatusByBill(AcrRecordOrg acr) {
        ...
        String numType = "";
        String transferData = "";
        if (ObjectUtil.isNotEmpty(key7)){
            numType = key7.getString("num_type");
            transferData = decodeBase64(key7.getString("transfer_data"));
        }
        statusInfo.setExt("");
...
//                statusInfo.setExt(privateBindInfoAxe.getTelXExt());
                statusInfo.setExt(acr.getDtmfKey());
        ...
        //对接联通集团总部问题字段处理
        if (StringUtil.isEmpty(statusInfo.getBindId())){
            statusInfo.setBindId(acr.getMessageId());
        }
        if (StringUtil.isEmpty(statusInfo.getTelX())){
            statusInfo.setTelX(acr.getMiddleNumber());
        }
        if (StringUtil.isEmpty(statusInfo.getExt())){
            statusInfo.setExt(acr.getDtmfKey());
        }
    ...
    public static PrivateFailMessage buildBillMessage(Bill billInfo) {
        PrivateFailMessage billMessage = new PrivateFailMessage();
        billMessage.setIp(PrivatePushApplication.ip);
        String billJson = JSON.toJSONString(billInfo);
        billMessage.setBody(billJson);
     ...
     * @return 通话状态信息
     */
    public static PrivateFailMessage buildStatusMessage(PrivateStatusInfo statusInfo) {
        PrivateFailMessage billMessage = new PrivateFailMessage();
        billMessage.setIp(PrivatePushApplication.ip);
...
//                privateBillInfo.setExt(privateBindInfoAxe.getTelXExt());
                privateBillInfo.setExt(acr.getDtmfKey());
            } else if (NumberTypeEnum.AXEYB_AXE.name().equals(numType)) {
                // 只接收没生成ayb的axeyb_axe关系
                PrivateBindInfoAxe privateBindInfoAxe =
                        JSON.parseObject(transferData, PrivateBindInfoAxe.class);
        ...
        //对接联通集团总部问题字段处理
        if (StringUtil.isEmpty(privateBillInfo.getBindId())){
            privateBillInfo.setBindId(acr.getMessageId());
        }
        if (StringUtils.isEmpty(privateBillInfo.getAreaCode())){
            privateBillInfo.setAreaCode(acr.getKey2());
        }
        if (privateBillInfo.getRecordFlag() == null){
        ...
        if (StringUtil.isEmpty(privateBillInfo.getRecordStartTime())){
            if (StringUtil.isNotEmpty(acr.getSrfmsgid())){
                privateBillInfo.setRecordStartTime(privateBillInfo.getBeginTime());
            }
        }
