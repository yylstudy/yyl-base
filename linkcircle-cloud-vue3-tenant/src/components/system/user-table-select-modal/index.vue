<template>
  <a-modal v-model:open="visible" :width="900" title="选择用户" @cancel="closeModal" @ok="onSelectEmployee">
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="手机号/邮箱" class="smart-query-form-item">
          <a-input style="width: 150px" v-model:value="params.phoneOrEmail" placeholder="手机号/邮箱" />
        </a-form-item>
...
<script setup>
  ...
  const props = defineProps({
    selectRoleId: {type:String}
  })
  // ----------------------- modal  显示与隐藏 ---------------------

  const visible = ref(false);
  async function showModal(id) {
    selectedRowKeyList.value = id || [];
    visible.value = true;
    onSearch();
  }
  function closeModal() {
  ...
  async function queryEmployee() {
    ...
    try {
      params.roleId = props.selectRoleId;
      let res = await userApi.queryNotInRoleUserByRoleId(params);
  ...
  function onSelectEmployee() {
    console.log(props.selectRoleId)
    console.log('=============================')
