    ...
    <a-form class="smart-query-form">
      <a-row class="smart-query-form-row">
        <a-form-item label="关键字" class="smart-query-form-item">
          <a-input style="width: 300px" v-model:value="queryForm.menuName" placeholder="菜单名称" />
        </a-form-item>
...
<!--          </a-button>-->
      ...
      >
        <template #bodyCell="{ text, record, column }">
          ...
          <template v-if="column.dataIndex === 'menuScope'">
            <a-tag >{{ getMenuScope( text) }}</a-tag>
          </template>
...
<!--          </template>-->
    ...
    <MenuOperateModal :menuList="menuList" ref="menuOperateModal" @reloadList="query" />
  </div>
</template>
<script setup>
  ...
  async function query() {
    try {
      ...
      menuList.value = responseModel.data;
      // 过滤搜索条件
  ...
  function getMenuScope(text){
    for(let index = 0;index<menuScopeList.value.length;index++){
      let menuScope = menuScopeList.value[index]
      if(menuScope.itemValue==text){
        return menuScope.itemText
      }
    }
  }
  ...
  function confirmBatchDelete(menuArray) {
    ...
    Modal.confirm({
      ...
      onOk() {
        console.log('OK');
        const idList = menuArray.map((e) => e.id);
    ...
    async function requestBatchDelete(idList) {
      SmartLoading.show();
      try {
        await menuApi.batchDeleteMenu(idList);
