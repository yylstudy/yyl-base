  ...
  <a-form class="smart-query-form">
    <a-row class="smart-query-form-row">
      <a-form-item label="字典编码" class="smart-query-form-item">
        <a-input style="width: 300px" v-model:value="queryForm.dictCode" placeholder="字典编码" />
      </a-form-item>
    ...
    <a-row class="smart-table-btn-block">
      <div class="smart-table-operate-block">
        ...
        <a-button
          ...
          v-privilege="'system:dict:delete'"
          type="text"
          danger
          size="small"
...
<!--        <a-button @click="cacheRefresh" v-privilege="'system:dict:refresh'" type="primary" size="small">-->
<!--          <template #icon>-->
<!--            <cloud-sync-outlined />-->
<!--          </template>-->
<!--          缓存刷新-->
    ...
    >
      <template #bodyCell="{ record, column }">
        <template v-if="column.dataIndex === 'dictCode'">
          <a @click="showValueList(record.id)">{{ record.dictCode }}</a>
        </template>
...
<script setup>
  import DictKeyOperateModal from './components/dict-operate-modal.vue';
  import DictValueModal from './components/dict-item-modal.vue';
  ...
  // 显示操作记录弹窗
  function showValueList(id) {
    dictValueModal.value.showModal(id);
  }
  ...
  async function ajaxQuery() {
    try {
      ...
      let responseModel = await dictApi.dictQuery(queryForm);
  ...
  const batchDelete = async () => {
    try {
      SmartLoading.show();
      await dictApi.delete(selectedRowKeyList.value);
