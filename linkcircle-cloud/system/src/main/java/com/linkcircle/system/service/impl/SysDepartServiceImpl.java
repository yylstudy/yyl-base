...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements SysDepartService {
     ...
     * 部门列表
     */
    @Cacheable(SysCacheConstant.DEPART_LIST_CACHE)
    @Override
    public List<SysDepartDTO> listAll() {
        return sysDepartMapper.listAll();
    }
    /**
     * 获取部门树形结构
     */
    @Cacheable(SysCacheConstant.DEPART_TREE_CACHE)
    @Override
    public List<SysDepartTreeDTO> departTree() {
        List<SysDepartDTO> list = getAllDepartCache();
    ...
    @CacheEvict(value = {SysCacheConstant.DEPART_LIST_CACHE, SysCacheConstant.DEPART_MAP_CACHE, SysCacheConstant.DEPART_SELF_CHILDREN_CACHE,
            SysCacheConstant.DEPART_TREE_CACHE, SysCacheConstant.DEPART_PATH_CACHE,}, allEntries = true)
    @Override
    public Result<String> add(SysDepartAddDTO dto) {
        SysDepart sysDepart = sysDepartMapStruct.convert(dto);
        SysDepart existsSysDepart =getByDepartName(dto.getName());
     ...
     *
     */
    @Cacheable(SysCacheConstant.DEPART_PATH_CACHE)
    @Override
    ...
    private SysDepart getByDepartName(String departName){
        LambdaQueryWrapper<SysDepart> wrapper = Wrappers.lambdaQuery();
    ...
    private List<SysDepartDTO> getAllDepartCache(){
